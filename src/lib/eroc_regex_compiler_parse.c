...
static int shift_instruction(eroc_regex_compiler_instance* inst, int ch)
{
    ...
    switch (ch)
    {
        ...
        case '*':
            retval = shift_star_instruction(inst);
            break;
    ...
    eroc_regex_compiler_instance* inst, int ch)
{
    switch (ch)
    {
        ...
        case 'f':
            return shift_literal_instruction(inst, '\f');
    ...
    eroc_regex_compiler_instance* inst, int ch)
{
    ...
    switch (ch)
    {
        ...
        case 's':
            (void)eroc_regex_ast_char_class_member_add(ast, ' ');
            (void)eroc_regex_ast_char_class_member_add(ast, '\t');
            (void)eroc_regex_ast_char_class_member_add(ast, '\r');
            (void)eroc_regex_ast_char_class_member_add(ast, '\n');
            (void)eroc_regex_ast_char_class_member_add(ast, '\f');
            (void)eroc_regex_ast_char_class_member_add(ast, '\v');
            break;
...
/**
 * \brief Shift a star instruction onto the stack.
 *
 * \param inst          The compiler instance for this operation.
 *
 * \returns 0 on success and non-zero on failure.
...
static int shift_star_instruction(eroc_regex_compiler_instance* inst)
{
    ...
    /* create a star node. */
    int retval = eroc_regex_ast_node_star_create(&ast, child);
    if (0 != retval)
    {
        return retval;
    }
...
/**
 * \brief Shift a plus instruction onto the stack.
 *
 * \param inst          The compiler instance for this operation.
 *
 * \returns 0 on success and non-zero on failure.
...
static int shift_plus_instruction(eroc_regex_compiler_instance* inst)
{
    ...
    /* create a plus node. */
    int retval = eroc_regex_ast_node_plus_create(&ast, child);
    if (0 != retval)
    {
        return retval;
    }
